...
export class PreguntaRepository {
    ...
    async crearPregunta(pregunta) {
        ...
        const query = `
            ...
            RETURNING *`;
        const values = [pregunta.preguntaTexto, pregunta.opcion1, pregunta.opcion2, pregunta.opcion3, pregunta.opcion4, pregunta.respuestaCorrecta];
    ...
    async actualizarPregunta(pregunta) {
        const query = `
            ...
            WHERE PreguntaId = $7
            RETURNING *`;
        const values = [pregunta.preguntaTexto, pregunta.opcion1, pregunta.opcion2, pregunta.opcion3, pregunta.opcion4, pregunta.respuestaCorrecta, pregunta.id];
        const { rows } = await this.DBClient.query(query, values);
        return rows[0];
    }
